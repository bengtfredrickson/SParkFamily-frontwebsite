...
class AttendanceReport extends Component {
    ...
    if (this.state.building !== null) {
      ...
      if (teacherresponse.status === 200) {
        await this.setState({
          ...
          teacher: teacherresponse.data.result[0].teacher_name,
        });
      }
  ...
  handleDropdownValues = async (event) => {
    ...
    if (this.state.building !== null || this.state.building !== undefined) {
      var teacherData = {
        createdBy: this.state.createdBy,
        buildingName: this.state.building,
      };
  ...
  getAttendanceData = async () => {
    ...
    const attendanceResponse = await studentMethods.getStudentAttendanceData(
      attendanceObject
    );
    ...
    if (attendanceResponse.status === 200) {
      var columnsList = [
        ...
        { name: "date", title: "Attendance Date" },
        { name: "attendance", title: "Attendance" },
      ];

      this.setState({
        studentData: attendanceResponse.data.result.students,
        filteredData: attendanceResponse.data.result.students,
      });
    }
    ...
    this.state.studentData.forEach((element) => {
      if (element.attendance !== "") {
        if (element.attendance === "Present") {
          presentCount.push(element);
        }
        if (element.attendance === "Absent") {
          absentCount.push(element);
        }
      }
    });

    if (presentCount.length !== 0 && absentCount.length !== 0) {
      this.setState({
        showChart: true,
        series: [presentCount.length, absentCount.length],
        options: {
          chart: {
            width: 260,
            type: "pie",
